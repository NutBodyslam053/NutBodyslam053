...
with DAG(
    ...
    dag.doc_md = """
        ...
        store it to the Google Cloud Storage (GCS), then upload to Google BigQuery.
    """
    ...
    t4 = GCSToBigQueryOperator(
        task_id='gcs_to_bq',
        bucket='asia-east2-datawarehouse-eab5d086-bucket',
        source_objects=['data/output.csv'],
        destination_project_dataset_table='datawarehouse.audible_data2',
        schema_fields=[
            {
                "mode": "NULLABLE",
                "name": "timestamp",
        ...
        write_disposition='WRITE_TRUNCATE', # Generate a new table every time, WRITE_APPEND; Append to an existing table.
)
...
# gsutil cp start_ws5_gcs_to_bq.py gs://asia-east2-datawarehouse-eab5d086-bucket/dags