...
def pcd_air4thai() -> None:
    ...
    def raw_area() -> None:
        from utils.utils import fetch_api_sync, upload_to_gcs_from_string

        @task
        def fetch_api_to_gcs_raw(ti=None) -> None:
            GCS_BASE_URIS = ti.xcom_pull(task_ids="init_constants.update_gcs_base_uris")

            json_data = fetch_api_sync(
                url=r"http://air4thai.pcd.go.th/services/getNewAQI_JSON.php",
    ...
    def staging_area(source_buckets: list[str], target_buckets: list[str]) -> dict[str, None]:
        from utils.pm25.pcd_air4thai_transform import TransformFunctions
    ...
    def data_warehouse(wh_types: list[str]) -> list[None, BigQueryInsertJobOperator]:
        ...
        chain(temp, *operators)

    warehouse = data_warehouse(wh_types=["station_info", "station_reads"])
